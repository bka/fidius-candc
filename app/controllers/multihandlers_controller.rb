class MultihandlersController < ApplicationController
  ...
  def index
    ...
    @interfaces = @worker.cmd_get_interfaces rescue nil
    @multihandlers = @worker.cmd_get_running_multihandler rescue nil
  end
  
  def create
    @worker = get_msf_worker
    @worker.cmd_start_multihandler :payload => params[:payload][:payload], :lport => params[:port], :lhost => params[:interface] #TODO: rescue nil
    redirect_to multihandlers_url  
  end
  
  def stop
    @worker = get_msf_worker
    @worker.cmd_stop_multihandler :jid => params[:jid]
    redirect_to multihandlers_url  
  end
