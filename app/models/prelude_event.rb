class PreludeEvent < ActiveRecord::Base
  ...
  def self.total_entries(options = nil)
    ...
    #options[:severity_source]
    #options[:severity_dest]
    
    return Prelude::Alert.total_entries
  end

  def self.find(*args)
    if args[0].is_a? Numeric
      a = Prelude::Alert.find(:all, :conditions => [ "_ident = ?", args[0] ])
      ...
      case args[0]  
        when :all
          ...
          return result
        when :first
          a = Prelude::Alert.first
          return PreludeEvent.new a
        when :last
          a = Prelude::Alert.last
          return PreludeEvent.new a
        else  
  ...
  def self.find_and_paginate_with_custom_options(page,severity = nil,perpage=nil,filter_source=nil,filter_dest=nil,from=nil,to=nil)
    ...
    if !from.nil?
      # TODO: Time.at rechnet bei umwandlung von timestamp nach TimeObjekt
      # offenbar 2h(zeitzone) drauf, deswegen
      # Time.new.utc_offset abziehen
      # gibts vtl. ne schönere lösung?
  ...
  #Prelude::DetectTime.find(:all, :conditions=>['time > 20100331103646']).length
  #ruby-1.8.7-p249 > PreludeEvent.find(:all, :conditions=>["time > 1269986400"], :joins=>[:detect_time,:impact,:source_address])
  #DateTime.strptime("2010-03-31 10:36:45", "%Y-%m-%d %H:%M:%S").to_time.to_i


  #PreludeEvent.find_and_paginate_with_custom_options(1,nil,10,'10.20.20.20','10.20.20.1',(1270031806-(60*60*2))).each do |pa| puts pa.detect_time.time end
end
